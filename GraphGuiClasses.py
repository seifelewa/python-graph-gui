...
class GraphScene(QtWidgets.QGraphicsScene):
    ...
    def display_path(self):
        if self.current_path_algo == 'PRIMS':  # if current path algorithm is prims
            ...
            if self.check_selected(2):  # if nodes are selected run DFS between them
                ...
                self.InvalidInMsg.setText('Must select 2 nodes to find shortest path')
                self.InvalidInMsg.exec_()
        elif self.current_path_algo == 'Greedy':  # if current algorithm is DFS
            if self.check_selected(2):  # if nodes are selected run DFS between them
                ...
                self.InvalidInMsg.setText('Must select 2 nodes to find shortest path')
                self.InvalidInMsg.exec_()
        elif self.current_path_algo == 'A*':  # if current algorithm is DFS
            if self.check_selected(2):  # if nodes are selected run DFS between them
    ...
    def add_node(self, event):
        ...
        if sure:  # dialog value was input
            if len(str(node_val)) < 5 and len(str(node_val)) > 0:  # if input was between 1 and 4 characters
                ...
                if ok: self.graph.add_heuristic(node.val,h)
            else:
    ...
    def remove_edge(self, node1_val, node2_val):
        ...
        if (node1_val, node2_val) not in self.edges:  # if edge from node1_val to node2_val not in edges dictionary
            if self.digraph or (
            node2_val, node1_val) not in self.edges:  # and edge from node2_val to node1_val not in edges dictionary
    ...
    def show_mst_prims(self):
        ...
        for edge in mst_edges:
            if (edge[1], edge[2]) in self.edges:  # and edge exists between current node value and next in path
                ...
                self.edges[(
                edge[2], edge[1])].highlighted = True  # else the edge exists as being from next in path to current node
            dist = dist + edge[0]
    ...
    def reset_path(self):
        ...
        elif self.current_path_algo == 'Greedy':  # if current algorithm is bellman_ford
            self.show_Greedy(self.path_displayed[1], self.path_displayed[2])  # run bellman ford
        elif self.current_path_algo == 'A*':  # if current algorithm is bellman_ford
            self.show_AStar(self.path_displayed[1], self.path_displayed[2])  # run bellman ford
