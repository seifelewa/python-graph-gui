...
def Astar(graph, from_v, to_v=None):
    ...
    while not q.empty():
        ...
        for adj_node in graph.nodes_dict[current[1]]:  # for each adjacent node
            ...
            if edge_w < 0: return [('Invalid', -1, None)]
            dist = current[0] + edge_w
            node_h = graph.heuristic_dict[adj_node]
            if node_h < 0: return [('Invalid', -1, None)]
            if adj_node not in visited:
                q.put((node_h+dist, adj_node))  # put new dist parent pair in queue
...
def Greedy(graph, from_v, to_v=None):
    ...
    while not q.empty():
        ...
        for adj_node in graph.nodes_dict[current[1]]:  # for each adjacent node
            node_h = graph.heuristic_dict[adj_node]
            # edge_w = graph[(current[1], adj_node)]
            if node_h < 0: return [('Invalid', -1, None)]
            # dist = current[0] + edge_w
            if adj_node not in visited:
